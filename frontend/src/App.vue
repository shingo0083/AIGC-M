            ...
            <div class="prompt-workspace card-shadow" v-if="currentManifest">
              <div class="workspace-header">
                ...
                <div class="actions">
                  ...
                  <el-button
                    v-if="isPhotographyStyle()"
                    type="warning" plain size="small" round
                    @click="handleCompilePhotography"
                    :loading="compiling">
                    ⚡ 优化(摄影)
                  </el-button>
                  <el-button
              ...
              <div class="prompt-editor" :class="{ 'is-outdated': isBlueprintOutdated }">
                <div v-if="isBlueprintOutdated" class="outdated-tip" @click="isPhotographyStyle() ? handleCompilePhotography() : handleCompile()">
                  <el-icon>
                    <Warning />
                  </el-icon> 配置已变更，点击此处刷新蓝图
                </div>
                <el-input :model-value="displayBlueprint" type="textarea" :rows="12" readonly
...
const compiledPromptPhoto = ref('')
const compiledPromptZImage = ref('')
const blueprintView = ref('photo') // 'photo' | 'zimage'
...
const handleStyleChange = async (id) => {
  ...
  // 切换风格必定导致旧蓝图失效
  compiledPromptPhoto.value = ''
  compiledPromptZImage.value = ''
  blueprintView.value = 'photo'
...
const toZImageBlueprint = (blueprint) => {
  if (!blueprint) return blueprint
  let out = blueprint.trimStart()

  // 1) Trigger words must be the first tokens for ComfyUI LoRA
  if (!out.toLowerCase().startsWith('sh0r7y_asian')) {
    out = `sh0r7y_asian\n${out}`
  }

  // 2) Remove face lock semantics (keep the rest of the photography blueprint intact)
  out = out.replace(/\*\*Face Identity \(Hard Constraint\):\*\*[\s\S]*?\n\n/g, '')

  // Replace Render Priority line to remove identity lock wording
  out = out.replace(/\*\*Render Priority:\*\*.*?\n/g,
    '**Render Priority:** Composition first; subject clarity next; lighting realism next; camera/film as secondary; physics/material must remain readable and physically consistent, supporting realism without becoming the narrative focus.\n'
  )

  // Remove facial-identity phrasing in body-consistency line (keep the constraint)
  out = out.replace(/Treat body shape as fixed like facial identity;\s*/g, 'Treat body shape as fixed; ')

  return out
}

const displayBlueprint = computed(() => {
  // Photography: allow two branches
  if (isPhotographyStyle()) {
    if (blueprintView.value === 'zimage') {
      return compiledPromptZImage.value || (compiledPromptPhoto.value ? toZImageBlueprint(compiledPromptPhoto.value) : computedDraftPrompt.value)
    }
    return compiledPromptPhoto.value || computedDraftPrompt.value
  }
  // Other styles: keep original behavior
  return compiledPromptPhoto.value || computedDraftPrompt.value
})

const copyPrompt = () => {
  navigator.clipboard.writeText(displayBlueprint.value)
...
const handleCompile = async () => {
  ...
  try {
    ...
    if (res.data.status === 'success') {
      compiledPromptPhoto.value = res.data.blueprint
      // keep z-image branch in sync
      compiledPromptZImage.value = toZImageBlueprint(res.data.blueprint)
      isBlueprintOutdated.value = false
...
const handleCompilePhotography = async () => {
  blueprintView.value = 'photo'
  await handleCompile()
}

const handleCompileZImage = async () => {
  blueprintView.value = 'zimage'
  // Ensure photo blueprint exists and is fresh
  if (!compiledPromptPhoto.value || isBlueprintOutdated.value) {
    await handleCompile()
  } else if (!compiledPromptZImage.value) {
    compiledPromptZImage.value = toZImageBlueprint(compiledPromptPhoto.value)
  }
  ElMessage.success('蓝图已切换为 z-image 版本（已添加触发词并移除人脸锁定语义）')
}
...
const handleGenerate = async () => {
  ...
  try {
    ...
    const finalPrompt = compiledPromptPhoto.value || computedDraftPrompt.value
