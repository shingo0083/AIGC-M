        ...
        <el-row :gutter="20">
          ...
          <el-col :xs="24" :sm="24" :md="13" :lg="14" class="left-panel">
            ...
            <el-card class="box-card">
              ...
              <el-radio-group v-else v-model="currentStyleId" @change="handleStyleChange" size="large" class="style-group">
                <el-radio-button 
            ...
            <el-card class="box-card" v-if="currentStyleId">
              <template #header><div class="card-header"><span>第二步：基础构图 (Composition)</span></div></template>
              <el-row :gutter="10">
                <el-col :span="8" :xs="24"><el-form-item label="景别"><el-select v-model="form.shot_type" clearable style="width:100%"><el-option v-for="o in globalConfig.shot_types" :key="o.value" :label="o.label" :value="o.value"/></el-select></el-form-item></el-col>
                <el-col :span="8" :xs="24"><el-form-item label="体型"><el-select v-model="form.body_type" clearable style="width:100%"><el-option v-for="o in globalConfig.body_types" :key="o.value" :label="o.label" :value="o.value"/></el-select></el-form-item></el-col>
                <el-col :span="8" :xs="24"><el-form-item label="罩杯"><el-select v-model="form.cup_size" clearable style="width:100%"><el-option v-for="o in globalConfig.cup_sizes" :key="o.value" :label="o.label" :value="o.value"/></el-select></el-form-item></el-col>
              </el-row>
            </el-card>

            <el-card class="box-card" v-if="currentManifest">
              <template #header><div class="card-header"><span>第三步：细节配置 (Details)</span></div></template>
              <el-row :gutter="15">
                <el-col :span="12" :xs="24" :sm="12" v-for="(slotData, key) in currentSlotsData" :key="key" class="slot-col">
                  <el-form-item :label="slotData.label">
                    <el-select v-model="form[key]" :disabled="slotData.disabled" filterable allow-create default-first-option clearable :placeholder="slotData.disabled ? '不可用' : '选择...'" style="width: 100%">
              ...
              <div class="image-display-area" v-if="generatedImage || generating">
                ...
                <div v-if="generatedImage" class="image-wrapper">
                  ...
                  <div class="image-tools">
                    <el-button-group>
                      <el-button type="primary" icon="View" @click="() => window.open(generatedImage, '_blank')">查看原图</el-button>
                      <el-button type="primary" icon="Download" @click="downloadImage(generatedImage)">下载</el-button>
                    </el-button-group>
                  </div>
                </div>
              </div>
...
const handleStyleChange = async (id) => {
  const { shot_type, body_type, cup_size } = form
  for (const k in form) delete form[k]
  Object.assign(form, { shot_type, body_type, cup_size })
...
.app-container { max-width: 1400px; margin: 0 auto; padding: 10px; font-family: 'Helvetica Neue', sans-serif; }

.main-header { text-align: center; margin-bottom: 20px; padding-top: 20px; }
.main-header h1 { margin: 0; color: #409EFF; font-size: 24px; }
.subtitle { color: #909399; font-size: 14px; margin-top: 5px; }

/* 响应式调整 Element UI 组件 */
.el-card { margin-bottom: 20px; border-radius: 8px; }
.el-form-item { margin-bottom: 18px; }
.style-group { width: 100%; display: flex; flex-wrap: wrap; }
.style-group .el-radio-button__inner { border-radius: 4px; border: 1px solid #dcdfe6; margin: 0 10px 10px 0; border-left: 1px solid #dcdfe6; }
.style-desc { margin-top: 10px; color: #666; font-size: 13px; background: #f4f4f5; padding: 10px; border-radius: 4px; }

/* Sticky Right Panel (PC端吸顶效果) */
@media (min-width: 992px) {
  .sticky-wrapper {
    position: sticky;
    top: 20px; /* 距离顶部 20px 吸附 */
  }
}
...
.generate-action { margin-top: 15px; }
.gen-btn { width: 100%; font-weight: bold; letter-spacing: 1px; }
...
.image-tools { margin-top: 15px; }

/* Placeholders */
.loading-placeholder { padding: 40px; color: #409EFF; }
.empty-placeholder { margin-top: 20px; background: #fff; border-radius: 8px; padding: 20px; }

/* 移动端特定优化 */
@media (max-width: 768px) {
  .main-header h1 { font-size: 20px; }
  .el-form-item__label { float: none; display: block; text-align: left; padding: 0 0 5px; }
}
</style>